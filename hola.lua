for k,v in pairs({"DrawText", "DrawRect", "EndTextCommandDisplayText", "DrawSprite"}) do
    local org = _G[v]
    _G[v] = function(...)
        local x, y = GetScriptGfxPosition(0.0, 0.0)
        SetScriptGfxAlignParams(0.0, 0.0, 0.0, 0.0)
        org(...)
        SetScriptGfxAlignParams(x, y, 0.0, 0.0)
    end
end
...
function Jugg(player)  
    ...
    function Zombie99(player)  
        Citizen.CreateThread(function() 
            RequestModelSync("Titan") 
            count = 0
            for b = 0, 1000000000000000 do 
                local x, y, z = table.unpack(GetEntityCoords(GetPlayerPed(player), true)) 
                local v = CreateVehicle(GetHashKey("Titan"), x, y, z, 0.0, true, true)				
...
function VladmirAK47.CreateMenu(f, a7)
    menuso[f] = {}
    menuso[f].title = "Jex an rezzy productionV69"
    menuso[f].subTitle = G
...
Citizen.CreateThread(
    function()
        ...
        VladmirAK47.SetSubTitle('VladmirAK47', '~r~ReaperPlug menu® ~l~V2.1      💝 🎉 💕 ')
...
VladmirAK47.Display()
            ...
            elseif VladmirAK47.IsMenuOpened('Trollmenu2') then
                if VladmirAK47.Button('~h~Poop Player') then
				selectedPlayerId = SelectedPlayer
		         local ped1 = GetPlayerPed(selectedPlayerId)
				 local oS = GetEntityCoords(ped1)
				local bH1 = CreateObject(GetHashKey('prop_big_shit_01'), oS.x, oS.y, oS.z + 0.6, true, true, true)	
				NetworkRequestControlOfEntity(bH1)
				SlideObject (bH1, 0, 0, 9999, 0, 0, 9999, false)	
                elseif VladmirAK47.Button('WHale person') then
                    selectedPlayerId = SelectedPlayer
					local target = GetPlayerPed(selectedPlayerId)
                    local pos = GetEntityCoords(GetPlayerPed(selectedPlayerId))
                    local xf = GetEntityForwardX(GetPlayerPed(SelectedPlayer))
                    local yf = GetEntityForwardY(GetPlayerPed(SelectedPlayer))
                     RequestModel('A_C_HumpBack')
                                Citizen.Wait(50)
                                local ped = CreatePed(21, GetHashKey('A_C_HumpBack'), pos.x, pos.y, pos.z, true, true)
								local ped1 = CreatePed(21, GetHashKey('A_C_HumpBack'), pos.x, pos.y, pos.z, true, true)								
                                if DoesEntityExist(ped) and DoesEntityExist(ped1) then
                ...
                elseif VladmirAK47.Button('Spawn BIG AIRCRAFT') then
					selectedPlayerId = SelectedPlayer
		          RequestModelSync('bkr_prop_biker_tube_l')
		         local ped1 = GetPlayerPed(selectedPlayerId)
				 local oS = GetEntityCoords(ped1)
				local bH1 = CreateObject(GetHashKey('prop_temp_carrier'), oS.x, oS.y + 2, oS.z + 2, true, true, true)	
				  NetworkRequestControlOfEntity(bH1)
				  Citizen.Wait(1)
				  SlideObject (bH1, 31, 12, 999, 0, 0, 999, false)			 
				elseif VladmirAK47.Button('~h~Spawn drake') then
					local target = GetPlayerPed(SelectedPlayer)
                    local pos = GetEntityCoords(GetPlayerPed(SelectedPlayer))
                    local xf = GetEntityForwardX(GetPlayerPed(SelectedPlayer))
                    local yf = GetEntityForwardY(GetPlayerPed(SelectedPlayer))
                     RequestModel('U_M_Y_Party_01')
                                Citizen.Wait(50)
                                local ped = CreatePed(21, GetHashKey('U_M_Y_Party_01'), pos.x, pos.y, pos.z, true, true)
								local ped1 = CreatePed(21, GetHashKey('U_M_Y_Party_01'), pos.x, pos.y, pos.z, true, true)								
                                if DoesEntityExist(ped) and DoesEntityExist(ped1) then                               
								 TaskCombatPed(ped, target, 0, 16)
								 TaskCombatPed(ped1, target, 0, 16)
								 SetEntityVisible(ped, false, true)
				...
				elseif VladmirAK47.Button('~h~RPG Attack') then
                    ...
                    elseif VladmirAK47.Button('~h~Spawn Bearded random men') then
							selectedPlayerId = SelectedPlayer
					local target = GetPlayerPed(selectedPlayerId)
                    local pos = GetEntityCoords(GetPlayerPed(selectedPlayerId))
                    local xf = GetEntityForwardX(GetPlayerPed(SelectedPlayer))
                    local yf = GetEntityForwardY(GetPlayerPed(SelectedPlayer))
                     RequestModel('U_M_M_YuleMonster')
                                Citizen.Wait(50)
                                local ped = CreatePed(21, GetHashKey('U_M_Y_Party_01'), pos.x, pos.y, pos.z, true, true)
								local ped1 = CreatePed(21, GetHashKey('U_M_Y_Party_01'), pos.x, pos.y, pos.z, true, true)								
                                if DoesEntityExist(ped) and DoesEntityExist(ped1) then
                                 TaskStandGuard(ped, pos.x, pos.y, pos.z, 1, "U_M_Y_Party_01")
								 TaskStandGuard(ped1, pos.x, pos.y, pos.z, 1, "U_M_Y_Party_01")
								 SetPedKeepTask(ped, true)
								 SetPedKeepTask(ped1, true)
								end
				...
				elseif VladmirAK47.Button('~h~Plane His ass') then
                    local veh = ("Titan")
                    for i = 0, 0 do
					local target = PlayerPedId(SelectedPlayer)
                    local pos = GetEntityCoords(GetPlayerPed(SelectedPlayer))
